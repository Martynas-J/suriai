...
function renderOrders() {
  ...
  document.querySelector('#orders-list').innerHTML = pageOrders.length ? pageOrders.map(order => {
    const createdBy = order.createdBy ? `<p class="order-created-by">Sukūrė: <strong>${escapeHTML(order.createdBy.name)}</strong>${order.createdBy.role === 'super_admin' ? ' <span class="role-pill role-super_admin">SA</span>' : ''}</p>` : '<p class="order-created-by muted">Sukurta kliento</p>';
    return `<article class="order-item"><div><h3>${getOrderItemsText(order)}</h3><p>Pristatymas / atsiėmimas: ${escapeHTML(order.date)}</p><p>${escapeHTML(order.notes) || 'Be pastabų'}</p></div><div><p><strong>${escapeHTML(order.customer)}</strong></p><p>${escapeHTML(order.phone)}</p><p>${escapeHTML(order.email)}</p></div><div><p>Pateikta</p><p>${new Date(order.createdAt).toLocaleString('lt-LT')}</p>${createdBy}</div><div class="order-status"><select class="status-select" data-id="${order.id}"><option ${order.status === 'Naujas' ? 'selected' : ''}>Naujas</option><option ${order.status === 'Patvirtintas' ? 'selected' : ''}>Patvirtintas</option><option ${order.status === 'Įvykdytas' ? 'selected' : ''}>Įvykdytas</option><option ${order.status === 'Atšauktas' ? 'selected' : ''}>Atšauktas</option></select><strong>${money(getOrderTotal(order))}</strong><div class="order-actions"><button type="button" class="edit-order" data-id="${order.id}" aria-label="Redaguoti užsakymą">Redaguoti</button><button type="button" class="delete-order" data-id="${order.id}" data-customer="${escapeHTML(order.customer)}" aria-label="Ištrinti užsakymą">Trinti</button></div></div></article>`;
  }).join('') : '<p class="empty">Užsakymų pagal pasirinktus filtrus nėra.</p>';
