...
function envSecret() {
  ...
  if (!value || value === 'change-me' || value === 'change-me-to-a-long-random-string-at-least-32-chars') return null;
  return value;
}
...
async function getSecret() {
  ...
  const rows = await query("SELECT `value` FROM settings WHERE `key` = 'session_secret'");
  cachedSecret = rows[0]?.value || 'change-me';
  return cachedSecret;
}
